CREATE OR REPLACE FUNCTION public.list_toppings_with_my_like_v2(_session_id text, _device_id uuid DEFAULT NULL::uuid, _limit integer DEFAULT 100)
 RETURNS TABLE(id uuid, session_id text, text text, kind text, prompt_id uuid, prompt_text text, pinned boolean, addressed boolean, likes integer, created_at timestamp with time zone, device_id uuid, role audience_role, op_id uuid, liked_by_me boolean)
 LANGUAGE sql
 STABLE
